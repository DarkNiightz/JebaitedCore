...
| **Stats Menu (/stats)**   | 54-slot **tabbed** GUI — **7 tabs:** Profile · Hub · SMP · **Hardcore · Arena · Chat games** · Achievements. Hub: **cosmetics unlock X/Y** per category; SMP: combat/world/mcMMO + **balance only on SMP worlds**; events: **per-key P/W/L** split across three buckets; Achievements: **12 milestone slots** + book. **`/stats` `/profile` `/pp`** any player; `<player>` offline OK. Hub **hotbar 9** → GUI. **`NetworkManager`** + **`ChatUtil`**. **Discord** `/player` embed. | ✅ Shipped |
...
| **Economy**               | In-game balance with `/balance`, `/pay`, and staff eco commands. **`/stats` wallet line / SMP tab economy** shown when the **viewer is on SMP** (overworld, nether, end — `WorldManager.isSmp`); not hub-primary.                                                                                                                                                                                                                               | ✅ Shipped |
...
| F   | Player Profile Overhaul — tabbed `/stats` + rich Discord `/player`; **shipped:** public `/stats` `<player>`, aliases, **7-tab** `StatsMenu` (Hardcore/Arena/Chat event buckets; cosmetics fractions; SMP-gated eco; achievement grid + book), hub hotbar head. **Remaining:** Adventure/`profile.labels`, bottom-row friend/party/settings/refresh, optional event overflow polish | All players            | Soon        | 🔧 In Progress                                                                                                                                           |
...
| I2  | Player Shops — player-to-player storefronts                                                                                                                                                                                                                                                                               | All players            | Later       | 🔧 In Progress (**V014** `player_shop_listings` + `[PlayerShopListingDAO](src/main/java/com/darkniightz/core/shop/PlayerShopListingDAO.java)`; GUI next) |
...
| **3** | **CTF team mode**                                   | Same-team PvP blocked (`[CtfTeamDamageListener](src/main/java/com/darkniightz/core/eventmode/CtfTeamDamageListener.java)`). **Shipped:** kits + strip/reapply; ground-flag `Item` pickup; **party-aware** `[TeamEngine](src/main/java/com/darkniightz/core/eventmode/team/TeamEngine.java)` + `PartyManager`. **Next:** HC-CTF rules, nametag colours. |
...
| 6   | [Player Profile Overhaul (/stats)](#6-player-profile-overhaul-stats)                                  | Large   | 🔧 In Progress — **7-tab** `StatsMenu` shipped; labels / bottom row remain                                      |
...
Replace the current 45-slot static `StatsMenu` with a tabbed, full-featured **Player Profile** screen. `/stats`, `/profile`, and `/pp` all open it — **public** for self or **`/stats <player>`** (any rank). Contains links to Settings, Friend invite, Party invite, and action buttons.

**Shipped slice (v1–v2):** `StatsMenu` is a **54-slot** GUI with tabs **Profile · Hub · SMP · Hardcore · Arena · Chat games · Achievements** (`plugin.yml` aliases **`/profile`** **`/pp`** → same executor as **`/stats`**). **`/stats <player>`** is **public** (no rank gate); **tab completion** lists online players (`canSee` for non-staff; **helper+** sees vanished names in the list). Hub tab: cosmetics **unlocked/total** per catalog category; SMP economy strip only when the viewer is on **SMP** worlds (overworld/nether/end); event keys split **chat vs hardcore vs arena** by config key naming; Achievements tab shows a **12-slot milestone snapshot** + book → **`AchievementsMenu`**; **`AchievementManager.ensureLoadedSync`** for offline targets. Skull → chat transcript (balance line respects SMP-only wallet). Remaining: Adventure-only text, `profile.labels.*`, bottom-row actions, optional event-lore pagination.
...
| `plugin.yml`                              | ✅ `profile` + `pp` aliases on `stats` (Bukkit binds aliases automatically; no extra `registerCommands` lines) |
| `AchievementManager`                      | ✅ `ensureLoadedSync(UUID)` for profile/ GUI reads                                                             |


### Rank gate

- Any player: `/stats`, `/stats <player>`, `/profile`, `/pp` (no permission gate — profile data is intentionally public server-side)
...
| `/mcability`, `/mccooldown`, `/ptp`                        | `[McMMODelegatedCommand](src/main/java/com/darkniightz/core/commands/McMMODelegatedCommand.java)` → mcMMO `PluginCommand` | **Wrapped / evicted**                    |
| `/mcc` (if used), `/xprate`, admin cmds                    | —                                                                                                                         | Pending / staff-only review              |
...
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- |
| mcMMO                  | Core overlaps: `/party`, `/p`, `/inspect` (+aliases), `/mcrank`, `/mcstats`, `/mctop`, `/mcability`, `/mccooldown`, `/ptp` (**Jebaited eviction + wrappers / delegation**) | Partial — see table above |
...
### I2 Player Shops — design contract
...
| Piece       | Direction                                                                                                                                                                                                                                                                                                                              |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **DB**      | `**V014__player_shop_listings.sql`** — `player_shop_listings` (`seller_uuid` → `players`, chest coords, `price_coins`, `quantity`, `item_bytes`, `active`, timestamps). Writes via `[PlayerShopListingDAO](src/main/java/com/darkniightz/core/shop/PlayerShopListingDAO.java)` (`insertListing`, `listActiveForSeller`, `deactivate`). |
...
- **Shipped (arena + UX slice):** `[EventArenaRegistry](src/main/java/com/darkniightz/core/eventmode/EventArenaRegistry.java)` + `[ArenaConfig](src/main/java/com/darkniightz/core/eventmode/ArenaConfig.java)` (`event_mode.arena_registry` in `config.yml`); KOTH hill + duration from registry; DB spawns still override YAML when present; `[TeamEngine](src/main/java/com/darkniightz/core/eventmode/team/TeamEngine.java)` / `[Team](src/main/java/com/darkniightz/core/eventmode/team/Team.java)` (**CTF:** party-cohesive teams via `PartyManager`; solos shuffle-split); `[CtfHandler](src/main/java/com/darkniightz/core/eventmode/handler/CtfHandler.java)` + `[CtfFlagListener](src/main/java/com/darkniightz/core/eventmode/CtfFlagListener.java)`; live event lines in `[ServerScoreboardManager](src/main/java/com/darkniightz/core/system/ServerScoreboardManager.java)`; `/event spectate` (helper+), `info`, `setreward`, `arenas`, `start <event> [arena]`.
- Lobby countdown polish (boss bar tuning, `/event leave` during lobby edge cases)
- **CTF follow-ups:** **Shipped:** ground-flag pickup (`Item` + PDC `ctf_ground_flag`, `[CtfGroundFlagListener](src/main/java/com/darkniightz/core/eventmode/CtfGroundFlagListener.java)`). **Shipped:** party-aware CTF assignment — `[TeamEngine.assignCtfTeams](src/main/java/com/darkniightz/core/eventmode/team/TeamEngine.java)` keeps party members on the same team (`PartyManager`). **Next:** HC-CTF rules, nametag colours *(kits + strip/restore: `ctf.red_kit` / `ctf.blue_kit`, `[CtfKitUtil](src/main/java/com/darkniightz/core/eventmode/CtfKitUtil.java)`; KOTH unc. leader in `[KothHandler](src/main/java/com/darkniightz/core/eventmode/handler/KothHandler.java)`)*
