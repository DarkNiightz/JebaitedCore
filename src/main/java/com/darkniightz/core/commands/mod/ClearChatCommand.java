...
public class ClearChatCommand implements CommandExecutor {
    ...
    private final AtomicLong lastClearAt = new AtomicLong(0L);
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        long remaining = (lastClearAt.get() + cooldownSec * 1000L) - now;
        if (!isConsole && remaining > 0) {
            long secs = (remaining + 999) / 1000;
            sender.sendMessage(Messages.prefixed("§eClearChat cooldown: §7wait §e" + secs + "§7s"));
            return true;
        }

        int blankLines = Bukkit.getPluginManager().getPlugin("JebaitedCore")
                .getConfig().getInt("moderation.clearchat.blank_lines", 150);
        String notice = Messages.prefixed("§7Chat was cleared by §e" + actorName);
        String filler = "§r";
