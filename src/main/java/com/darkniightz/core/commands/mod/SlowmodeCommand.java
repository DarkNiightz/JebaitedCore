...
public class SlowmodeCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length < 1) {
            sender.sendMessage(Messages.prefixed("§eUsage: §f/" + label + " <seconds|off>"));
            return true;
        }

        if (args[0].equalsIgnoreCase("off") || args[0].equals("0")) {
            moderation.setSlowmodeSeconds(0);
            sender.sendMessage(Messages.prefixed("§eSlowmode §7is now §cOFF"));
            return true;
        }
        ...
        try {
            ...
            if (seconds < 0) {
                sender.sendMessage(Messages.prefixed("§cSeconds must be >= 0"));
                return true;
            }
            moderation.setSlowmodeSeconds(seconds);
            sender.sendMessage(Messages.prefixed("§eSlowmode §7set to §e" + seconds + "s"));
        } catch (NumberFormatException e) {
            sender.sendMessage(Messages.prefixed("§cInvalid number: §e" + args[0]));
        }
        return true;
    }
}
