...
public class ReloadCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (sender instanceof Player p) {
            ...
            if (!allowed) {
                sender.sendMessage("§cThis command is restricted to developers and above.");
                return true;
            }
        }
        ...
        try {
            ...
            sender.sendMessage("§aJebaitedCore reloaded. §7(" + took + "ms)");
        } catch (Exception ex) {
            sender.sendMessage("§cReload failed: §7" + ex.getMessage());
