...
public class MessageCommand implements CommandExecutor {
    ...
    public MessageCommand(ProfileStore profiles, RankManager ranks, MessageManager messageManager) {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length < 2) {
            player.sendMessage(Messages.prefixed("§eUsage: §f/" + label + " <player> <message>"));
            return true;
        }
        ...
        if (target == null || !target.isOnline()) {
            player.sendMessage(Messages.prefixed("§cThat player is not online."));
            return true;
        }
        if (target.getUniqueId().equals(player.getUniqueId())) {
            player.sendMessage(Messages.prefixed("§cYou cannot message yourself."));
            return true;
        }
        ...
        player.sendMessage(Messages.prefixed("§dDM sent §8→ §f" + targetName + "§7: " + message));
        target.sendMessage(Messages.prefixed("§dIncoming DM §8← §f" + senderName + "§7: " + message));
