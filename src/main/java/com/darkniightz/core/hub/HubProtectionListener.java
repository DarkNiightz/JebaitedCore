...
public class HubProtectionListener implements Listener {
    ...
    public HubProtectionListener(Plugin plugin, ProfileStore profiles, RankManager ranks, SpawnManager spawnManager) {
        this.plugin = plugin;
        this.profiles = profiles;
        this.ranks = ranks;
        this.spawnManager = spawnManager;
        this.worldManager = plugin instanceof JebaitedCore core
                ? core.getWorldManager()
                : new WorldManager(plugin, new com.darkniightz.core.world.WorldConfigManager(plugin));
    }
    ...
    public void onPlace(BlockPlaceEvent event) {
        if (!worldManager.isHub(event.getBlock().getWorld())) return;
    ...
    public void onHubCombatPrep(PlayerInteractEvent event) {
        if (!(event.getPlayer() instanceof org.bukkit.entity.Player player) || !worldManager.isHub(player)) return;
        Action action = event.getAction();
        if (action != Action.LEFT_CLICK_AIR) return;
        Material held = event.getMaterial();
        String name = held == null ? "AIR" : held.name();
        if (held == null || held.isAir() || name.contains("SWORD") || name.contains("AXE") || name.contains("HOE") || held == Material.TRIDENT) {
            event.setCancelled(true);
        }
    }
    ...
    public void onDamage(EntityDamageEvent event) {
        ...
        if (event.getEntity() instanceof org.bukkit.entity.Player player && worldManager.isHub(player)) {
            event.setCancelled(true);
        }
    }
    ...
    public void onRespawn(PlayerRespawnEvent event) {
        ...
        Location spawn = spawnManager == null ? null : spawnManager.getSpawnForWorld(event.getPlayer().getWorld().getName());
        if (spawn == null) spawn = event.getPlayer().getWorld().getSpawnLocation();
        if (spawn == null && !Bukkit.getWorlds().isEmpty()) spawn = Bukkit.getWorlds().get(0).getSpawnLocation();
    ...
    public void onEntityExplode(EntityExplodeEvent event) {
        ...
        if (!worldManager.isHub(event.getLocation().getWorld())) return;
    ...
    public void onEntityTargetPlayer(EntityTargetLivingEntityEvent event) {
        ...
        if (event.getTarget() instanceof org.bukkit.entity.Player player && worldManager.isHub(player)) {
            event.setCancelled(true);
        }
    }
}
