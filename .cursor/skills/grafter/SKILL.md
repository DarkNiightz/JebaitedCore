...
- **Execution:** Apply `.env`, `docker-compose.yml`, and config edits in the workspace yourself; follow [`.cursor/rules/jebaited-execution.mdc`](../../rules/jebaited-execution.mdc) and [`.cursor/rules/jebaited-plugin-standards.mdc`](../../rules/jebaited-plugin-standards.mdc) (ROADMAP + `SESSION_HANDOFF.md` continuity). Do not offload trivial "add this line" steps to Jamie unless a secret is not in the repo.
